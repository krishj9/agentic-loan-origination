...
- Name: loan_application_flow.spec.ts >> Application Form >> submit button is visible
- Location: tests/e2e/loan_application_flow.spec.ts:221:3
...
Locator: getByRole('button', { name: /create application/i })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /create application/i })
...
> 223 |     await expect(submitBtn).toBeVisible();
  ...
  316 |     await expect(outcome).toBeVisible({ timeout: 10_000 });
  317 |   });
  318 | });
  319 | 
```